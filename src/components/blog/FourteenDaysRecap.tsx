...
export function FourteenDaysRecap({ highlights }: { highlights: RecapHighlight[] }) {
  return (
    <section className="rounded-2xl border border-border bg-card p-5">
      <h3 className="text-lg font-semibold text-foreground">14 Days of USDM: The Fast Recap</h3>
      <p className="mt-1 text-sm text-(--text-secondary)">From launch day to six-figure TVL — here&apos;s how fast the momentum built.</p>
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {highlights.map((item) => (
          <article key={item.day} className="rounded-xl border border-border bg-(--surface-soft) p-4">
            ...
            <p className="mt-2 text-xs text-(--text-muted)">{item.metricLabel}</p>
